...
def _default_harness(coverage_source: list[str] | None = None) -> HarnessConfig:
    """The stock Python harness (§2.1), used when `config.json` has none.
    ...
    ``ruff`` is reachable only as ``python -m ruff``.
    """
    return HarnessConfig(
        ...
        coverage_source=coverage_source if coverage_source else ["src"],
        coverage_floor=0.85,
        timeout_s=900,
    )


def _config_section(root: Path, name: str) -> dict[str, JsonValue]:
    """One top-level object from `config.json`, or empty when absent.

    Never raises on a malformed file: a user setting this up for the first
    time passes through every broken state, and a traceback teaches them
    nothing (§11.2 rule 4). An unreadable config falls back to the default
    harness, whose coverage source is then validated like any other.
    """
...
def _load_harness(root: Path) -> HarnessConfig:
    """The harness for this repository: `config.json` first, default second.

    §7.3 is explicit that harness commands come from configuration only, and
    until now nothing in the CLI read any -- ``_default_harness`` was applied
    unconditionally, so a target repository could not configure its own lint,
    test or coverage commands at all.

    **The coverage source is validated against the repository**, and this is
    the part that matters. The old hardcoded ``["src"]`` is a guess about
    layout, and when it is wrong ``coverage`` collects nothing, the gate
    reports ``harness_error: no coverage data was produced by the harness``,
    and every implementation task fails for a reason that has nothing to do
    with the agent's work. That was found by pre-flighting a real run: it
    would have escalated a task after three *paid* dispatches.

    So a source that does not exist on disk is now a loud ``E_HARNESS_ERROR``
    naming the fix, never a silent no-data run.

    **Why the source is required in configuration rather than derived from
    the plan.** Deriving it from a task's ``writable_globs`` (or from any
    other plan field) would route a *plan-authored* value into a subprocess
    argv, and the plan is generated by an agent. §7.3 -- "No agent-supplied
    value reaches ``cmd``. Harness commands come from ``config.json`` only" --
    and §11.1 rule 2 both forbid exactly that, and the convenience is not
    worth putting agent output on a command line. ``redgear init`` writes a
    real ``config.json``, so a user never meets the error in practice.
    """
    commands = _config_section(root, "commands")
    gates = _config_section(root, "gates")

    configured_source = _string_list(commands.get("coverage_source"))
    harness = _default_harness(configured_source)

    overrides: dict[str, object] = {}
    for key in ("lint_cmd", "test_cmd", "coverage_cmd"):
        command = _string_list(commands.get(key))
        if command is not None:
            overrides[key] = command
    floor = gates.get("coverage_floor")
    if isinstance(floor, (int, float)) and not isinstance(floor, bool) and 0.0 <= floor <= 1.0:
        overrides["coverage_floor"] = float(floor)
    timeout = gates.get("timeout_s")
    if isinstance(timeout, int) and not isinstance(timeout, bool) and timeout >= 1:
        overrides["timeout_s"] = timeout
    if overrides:
        harness = harness.model_copy(update=overrides)

    missing = [source for source in harness.coverage_source if not (root / source).exists()]
    if missing:
        raise UnsafeHarnessCommandError(
            "the configured coverage source does not exist in this repository; "
            "coverage would collect nothing and every implementation task would "
            "fail on coverage_delta",
            detail={
                "missing": ", ".join(missing),
                "configured_in": str(config_path(root))
                if configured_source
                else "built-in default (no commands.coverage_source in config.json)",
                "fix": f'set "commands": {{"coverage_source": ["<dir>"]}} in {config_path(root)}',
            },
        )
    return harness
...
def init(repo: RepoOption = None) -> None:
    ...
    try:
        ...
        written = state_engine.write_default_config(root, _starter_config(root))
    except RedgearError as error:
        raise _fail(error) from error

    console.print(f"[green]initialised[/green] {created}")
    console.print(f"  wrote [dim]{written}[/dim]")
    source = _guess_coverage_source(root)
    if source is None:
        console.print(
            "  [yellow]set commands.coverage_source[/yellow] to the directory holding "
            "your package; no obvious candidate was found"
        )
    else:
        console.print(f"  coverage source: [dim]{source}[/dim] (edit if that is wrong)")
    console.print("Next: add a plan, review it, then `redgear run`.")


#: Directories a Python project's measurable code usually lives in. Used ONLY
#: to seed `config.json` at init time, where a human reads the result and can
#: correct it -- never consulted at run time, where §7.3 requires the value to
#: come from configuration rather than from a guess.
_COVERAGE_CANDIDATES: tuple[str, ...] = ("src", "lib", "app")


def _guess_coverage_source(root: Path) -> str | None:
    """A plausible coverage source for a fresh repository, or None.

    Prefers a conventional container directory, then falls back to the single
    top-level Python package if there is exactly one. Ambiguity returns None
    rather than picking: a wrong guess written into config is worse than an
    empty field the user is told to fill, because the wrong one runs.
    """
    for candidate in _COVERAGE_CANDIDATES:
    ...
    packages = [
        ...
        and entry.name not in {"tests", "test", "docs", "node_modules"}
        and any(entry.glob("*.py"))
    ]
    return packages[0] if len(packages) == 1 else None


def _starter_config(root: Path) -> dict[str, JsonValue]:
    """The `config.json` `redgear init` writes.

    Written with the real commands filled in rather than left for the user to
    discover, because the failure mode of an absent harness config is a task
    that fails on `coverage_delta` for reasons unrelated to the agent's work
    -- after paying for the dispatches that got there.
    """
    harness = _default_harness()
    source = _guess_coverage_source(root)
    commands: dict[str, JsonValue] = {
        "lint_cmd": list(harness.lint_cmd),
        "test_cmd": list(harness.test_cmd),
        "coverage_cmd": list(harness.coverage_cmd),
        "coverage_source": [source] if source else [],
    }
    return {
        "runner": {"executable": "claude", "bare": False},
        "commands": commands,
        "gates": {"coverage_floor": harness.coverage_floor, "timeout_s": harness.timeout_s},
    }
...
def run(
    ...
    # The spend cap lives on `Budget` (§8.1) but is enforced by the agent CLI,
    # so it has to be handed to the adapter. It was not: `ClaudeCodeConfig`
    # was built with the executable alone, leaving `per_turn_usd` at None
    # forever -- so setting a cap on the Budget did nothing at all, and the
    # `--max-budget-usd` flag was never emitted.
    config = (
        ClaudeCodeConfig(executable=resolved_executable, per_turn_usd=budget.per_turn_usd)
        if resolved_executable is not None
        else ClaudeCodeConfig(per_turn_usd=budget.per_turn_usd)
    )
    ...
    if budget.per_turn_usd is not None:
        console.print(f"  spend cap: [dim]${budget.per_turn_usd:.2f} per dispatch[/dim]")
    console.print()

    try:
        outcome = orchestrator.run(root, runner=agent, budget=budget, harness=_load_harness(root))
    except RedgearError as error:
...
def _dry_run(root: Path, *, only: str | None) -> None:
    ...
    try:
        composed = orchestrator.compose_dry_run(root, harness=_load_harness(root))
    except RedgearError as error:
        raise _fail(error) from error
...
def doctor(repo: RepoOption = None) -> None:
    ...
    # `doctor` REPORTS a bad harness rather than exiting on one -- diagnosing
    # exactly this is what the command is for, and a doctor that dies on the
    # illness is no use.
    try:
        harness = _load_harness(root)
        coverage_status = "[green]" + ", ".join(harness.coverage_source) + " exists[/green]"
    except RedgearError as error:
        healthy = False
        harness = _default_harness()
        coverage_status = f"[red]{error.code}[/red] {error.detail.get('missing')} not found"
    ...
    # The check that would have saved three paid dispatches: a coverage source
    # that does not exist collects nothing, and `coverage_delta` then fails
    # every implementation task for a reason unrelated to the agent's work.
    table.add_row("harness: coverage source", coverage_status)
